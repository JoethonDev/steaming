  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (searchMode === "id") {
        ...
        router.push(`/${locale}/player/${keyword}`);
        return;
      } else {
        // Search by name - navigate to search results page with just the query
        router.push(`/${locale}/dashboard/search?q=${encodeURIComponent(keyword)}`);
        return;
      }
    } catch (err: any) {
        ...
        <header className="flex items-center justify-between">
          <div>
            <h1 className="text-4xl font-black tracking-tight mb-2">
              {t('welcome_back')}, {session?.user?.email?.split("@")[0]}!
            </h1>
            <p className="text-slate-400">
              {t('streaming_command_center')}
            </p>
          </div>
          ...
          <div className="bg-[#0f1116] border border-white/10 p-8 rounded-3xl">
            ...
            <form id="series-search-form" onSubmit={handleSearch} className="space-y-3">
              <input
                ...
                placeholder={searchMode === "name" ? t('series_name') : t('series_id')}
            ...
            <div className="p-6 space-y-3">
              {history?.recentSeries && history.recentSeries.length > 0 ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-slate-500">{locale === 'ar' ? 'لا توجد مسلسلات حديثة' : 'No recent series'}</p>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="p-6 space-y-3 max-h-80 overflow-y-auto custom-scrollbar">
              {history?.recentEpisodes && history.recentEpisodes.length > 0 ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-slate-500">{locale === 'ar' ? 'لا توجد حلقات حديثة' : 'No recent episodes'}</p>
                </div>
              )}
            </div>
          </div>
